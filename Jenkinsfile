...
pipeline {
    ...
    stages {
        stage('Website') {
            ...
            steps {
                // The @deadlinks/cargo-deadlinks* archives are generated by github-release-binary.js and
                // cached by locator hash only, so a node holding a stale copy fails yarn install with
                // YN0018 forever. Evict them so they are rebuilt from the GitHub release on every build.
                sh 'cd $WORKSPACE/camel-website && rm -f "$(yarn config get cacheFolder)"/@deadlinks-cargo-deadlinks*'
