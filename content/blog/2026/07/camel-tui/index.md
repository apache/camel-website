...
![Camel TUI Diagram — route topology with seven interconnected routes](./camel-tui-diagram.png)
...
![Camel TUI Inspect — stepping through an exchange with headers, body, and BHPV change indicators](./camel-tui-inspect.png)
...
![Camel TUI Send Message — sending a test message to a route with JSON body](./camel-tui-send.png)
...
Every screenshot in this blog post was generated by an AI agent (Claude Code) controlling the
TUI through its embedded MCP server. The agent started the TUI with `camel tui --mcp`, connected
as an MCP client, then navigated tabs, selected routes, typed text into input fields, and
triggered the built-in screenshot action — all programmatically, without a human touching
the keyboard. The TUI saved each screenshot as SVG, and the agent converted them to PNG.
